...
const createMetricsBundle = () => {
  ...
  return {
    ...
    trackDurationSeconds: new Histogram({
      ...
      // audio, transcoded from Plex) followed by an aubio tempo analysis,
  ...
  inflightRefresh = (async () => {
    try {
      const [
        ...
        prisma.popularity.count({ where: { provider: "not_found" } }),
        ...
        prisma.audioFeature.count({
          where: { energy: null, valence: null, danceability: null, tempo: null },
        }),
        prisma.audioFeature.count({ where: { tempo: { not: null } } }),
        prisma.track.count({ where: { tagsSyncedAt: { not: null } } }),
        prisma.track.count({
          where: { tagsSyncedAt: { not: null }, tags: { some: { type: "genre" } } },
        }),
